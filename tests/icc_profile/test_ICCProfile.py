...
def test_iccprofile_get_info():
    ...
    expected_result[6] = ["Created", strftime("%Y-%m-%d %H:%M:%S", icc.dateTime.timetuple())]
    expected_result[17] = ["Checksum", "0x%s" % binascii.hexlify(icc.ID).upper().decode()]
...
def test_hexrepr_is_with_mapping_supplied():
    """testing if the ``DisplayCAL.ICCProfile.hexrepr`` is working as expected when a mapping is supplied
    """
...
def test_hexrepr_is_without_mapping_supplied():
    """testing if the ``DisplayCAL.ICCProfile.hexrepr`` is working as expected when no mapping is supplied
    """
    from DisplayCAL.ICCProfile import hexrepr, cmms
    test_bytes_string = b"ADBE"
